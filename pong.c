...
void showScene(int xl, int xr, int yl, int yr, int xball, int yball, int scoreLeft, int scoreRight);
void startGame();
...
void startGame() {
    ...
    showScene(xDefLeft, xDefRight, yDef, yDef, xBall, yBall, scoreLeft, scoreRight);
    ...
    while (choice != 'Q' && choice != 'q') {
        ...
        if ((choice == 'A' || choice == 'a') && next == '\n') {
            yLeft = moveRocketUp(yLeft, speed);
        }
 
        if ((choice == 'Z' || choice == 'z') && next == '\n') {
            yLeft = moveRocketDown(yLeft, speed);
        }
 
        if ((choice == 'K' || choice == 'k') && next == '\n') {
            yRight = moveRocketUp(yRight, speed);
        }
 
        if ((choice == 'M' || choice == 'm') && next == '\n') {
        ...
        if (scoreLeft == 21 || scoreRight == 21) {
            ...
            showScene(xDefLeft, xDefRight, yLeft, yRight, xBall, yBall, scoreLeft, scoreRight);
    }
}
...
int checkCollisionL(int x, int y, int xl, int yl, int mode) {
    if ((x == xl + 1 || x == xl - 1) && y < yl + 2 && y > yl - 2) mode = mode * (-1);
 
    return mode;
}
 
int checkCollisionR(int x, int y, int xr, int yr, int mode) {
    if ((x == xr + 1 || x == xr - 1) && y < yr + 2 && y > yr - 2) mode = mode * (-1);
